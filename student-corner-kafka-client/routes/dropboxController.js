...
var signupData = require('./sign_up');
...
exports.registerUser = function (req, res) {
    console.log('Hello frm registerUser***');
    console.log('Hello***', req.body.userEmail);

    var id = req.body.userEmail;
    var pwd = bcrypt.hashSync(req.body.password);
    var fn = req.body.firstName;
    var ln = req.body.lastName;
    var bdate = req.body.dob;
    ...
    var sqlQuery = "insert into users (userId, userEmail, userPassword, userFirstName, userLastName, userBDate, userGender, userWork, userEducation, userInterests) values ('" + id + "', '" + id + "', '" + pwd + "', '" + fn + "', '" + ln + "', '" + bdate + "', '" + gender + "', '" + work + "', '" + edu + "', '" + inter + "')";
    ...
    console.log("KAFKA-CLIENT msg_payload=>:", msg_payload);

    kafkaClient.make_request(req_topics.REGISTER, res_topics.REGISTER, msg_payload, function (err, result) {
        console.log("sdskugjgds", err);
        console.log("sgyjsd", result);
...
exports.validateLogin = function (req, res) {
    ...
    console.log("passport call", req.body);
...
exports.fileUpload = function (req, res) {
    ...
    try {
        if (req.session.username !== undefined || req.session.username !== "") {
            ...
            var busboy = Busboy({ headers: req.headers }); //new
...
// Added new code for posts  -11/22/2022
exports.createPost = function (req, res) {
    //console.log('request in createPost', req.body.selectedFile);    
    ...
    try {
        kafkaClient.make_request(req_topics.CREATE_POSTS, res_topics.CREATE_POSTS, msg_payload, function (err, results) {
            if (err) {
                res.status(400).send();
            } else {
                res.status(200).send(results)
...
exports.listPost = function (req, res) {
    //console.log('request in listPost', req);  
    var msg_payload = {
    };
    try {
        kafkaClient.make_request(req_topics.LIST_POSTS, res_topics.LIST_POSTS, msg_payload, function (err, results) {
            if (err) {
                res.status(400).send();
            } else {
                res.status(200).send(results)
...
// Added new code for fetchPostDetails  -11/23/2022
exports.fetchPostDetails = function (req, res) {
    //console.log('hey');
    //console.log('request in fetchPostDetails', req.params['id']);
    var msg_payload = {
        id: req.params['id']
    };
    try {
        kafkaClient.make_request(req_topics.FETCH_POST, res_topics.FETCH_POST, msg_payload, function (err, results) {
            if (err) {
                res.status(400).send();
            } else {
                res.status(200).send(results)
...
exports.deletePostDetails = function (req, res) {
    console.log('hey');
    console.log('request in deletePostDetails', req.params['id']);
    var msg_payload = {
        id: req.params['id']
    };
    try {
        kafkaClient.make_request(req_topics.DELETE_POST, res_topics.DELETE_POST, msg_payload, function (err, results) {
            if (err) {
                res.status(400).send();
            } else {
                res.status(200).send(results)
...
// Added new code for fetchPostDetails  -11/23/2022
exports.fetchUserDetails = function (req, res) {
    //console.log('hey');
    //console.log('request in fetchUserDetails', req.params['id']);
    var msg_payload = {
        id: req.params['id']
    };
    try {
...
exports.createComment = function (req, res) {
    console.log('create Comment in client ',req.body)
    var comment = req.body.comment;
    var postId = req.body.postId;
    var author = req.body.userEmail;
    ...
    try {
        kafkaClient.make_request(req_topics.CREATE_COMMENT, res_topics.CREATE_COMMENT, msg_payload, function (err, results) {
            if (err) {
                res.status(400).send();
            } else {
                res.status(200).send(results)
...
exports.listComment = function (req, res) {
    ...
    console.log('request in listComment', req.params['id']);       

    var msg_payload = {
        postId: req.params['id']
    };
    try {
        kafkaClient.make_request(req_topics.GET_COMMENTS, res_topics.GET_COMMENTS, msg_payload, function (err, results) {
            if (err) {
                res.status(400).send();
            } else {
                res.status(200).send(results)
